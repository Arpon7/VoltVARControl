...
# TASRL projected gradient flow, and paper-weighted ΔF. Adds --paper_strict mode
# that uses a single, non-escalating disturbance per trial (closer to the base paper).
...
# Disturbance protocol (default mode)
from disturbance_gen import paper_scenarios
...
def _apply_single_kick(env, rng: np.random.Generator,
    ...
    if hasattr(net, "sgen") and len(net.sgen):
        ...
        # leave sgen q_mvar as-is (PV usually P-only in these test feeders)
    if hasattr(env, "_runpp"): env._runpp(init_dc=False)
    # Optionally ensure a violation exists; if not, nudge once more lightly
...
def run_trial(env,
    ...
    deltaF_true = F(q*, v(q*)) - F(0, v(0))  using paper-style X^{-1} weighting.
    """
    ...
    for _ in range(steps):
        # 1) measure
        v_ctrl = _get_ctrl_voltages(env)
        # 2) act (safe gradient flow)
        q = tasrl.act(v_ctrl)
        q = np.asarray(q, dtype=float).reshape(-1)
        # 3) step env
        # env.step_Preward expects ΔQ; TASRL returns |Q| setpoints
        delta_q = q - q_prev
        q_prev = q
        obs, r, done, _info = env.step_Preward(delta_q)
        reward_sum += float(r)
        t += dt_s
        last_v = v_ctrl
        if recovery is None and np.all((v_ctrl >= v_band[0]) & (v_ctrl <= v_band[1])):
            recovery = t
        if done:
            break
    ...
    F_star = paper_F(q=tasrl._q, v=last_v, eta_over_sbar=tasrl.eta_over_sbar, X_inv=X_inv)
    ...
    try:
        if hasattr(env, "network") and hasattr(env.network, "sgen") and len(env.network.sgen):
            ...
            F0 = paper_F(q=np.zeros(n, dtype=float), v=v0, eta_over_sbar=tasrl.eta_over_sbar, X_inv=X_inv)
        else:
            v0 = last_v
            F0 = paper_F(q=np.zeros(n, dtype=float), v=v0, eta_over_sbar=tasrl.eta_over_sbar, X_inv=X_inv)
    finally:
        ...
        if saved_q:
            ...
            if hasattr(env, "_runpp"): env._runpp(init_dc=False)
            if hasattr(env, "network") and hasattr(env, "v") and hasattr(env.network, "res_bus"):
                try:
                    env.v = np.asarray(env.network.res_bus["vm_pu"].values, dtype=float)
                except Exception:
                    pass

    deltaF_true = F_star - F0
    ...
    if paper_strict:
        ...
        for _ in range(trials):
            ...
            try:
                ...
                tasrl.reset(q0=np.zeros(n))
    q_prev = np.zeros(n, dtype=float)
                rtime, treward, dF = run_trial(env, tasrl,
        ...
        # DEFAULT: your original scenario generator
...
if __name__ == "__main__":
    ...
    ap.add_argument("--case", type=int, choices=[13, 14, 123], required=True)
    ...
    out = evaluate(case=args.case, trials=args.trials, dt=args.dt, horizon=args.horizon,
                   ...
                   ctrl_str=args.ctrl, paper_strict=args.paper_strict)
